...
const Suggestions = () => {
    ...
    return (
        <div className="suggestions bg-secondary mt-10 rounded-lg p-3 hidden md:flex flex-col gap-6 md:sticky top-28">
            <h2 className={'text-lg'}>Suggestions for you</h2>
            <div className="all-suggestions flex flex-col gap-4">
                {
                    users.filter(user => user.username !== userDetails.username && !user.followers.find(user => user._id === userDetails._id)).map(user => (
                        <div key={user._id} className="ind-suggestion flex items-center justify-between gap-5">
                            ...
                            <div className="button">
                                <button className={'bg-button text-sm px-4 py-2 rounded-3xl flex items-center justify-center gap-1'} onClick={()=>followUser(user._id)}>
